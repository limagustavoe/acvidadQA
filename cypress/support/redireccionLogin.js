 Cypress.Commands.add('redirigirAlLoginDespuesDelRegistro', () => {
    ...
    cy.get('[data-cy="input-dni"]').clear().type('51268800') // tuve que usar un dni distinto al registrado
    ...
    cy.get('[data-cy="input-email"]').clear().type('juan.perez91@example.com.ar')
    cy.get('[data-cy="input-confirmar-email"]').clear().type('juan.perez91@example.com.ar')   // Usé un email distinto al registrado previamente
    ...
    cy.log('Luego del registro, el usuario es redirigido al login')
